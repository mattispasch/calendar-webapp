	...
	if (oauth2Client != null) {
		...
		request(syncInfoUrl, function(err, res, body) {
			...
			if (err) {
				...
				var baseUrl = req ? req.protocol + "://" + req.get('host') : "http://localhost:3000";
...
exports.getCalendarByGoogleId = function(googleId, callback) {
	loadClientAndAuthClient(null, function(err, client, authClient) {
		...
		client.calendar.calendars.get(googleId).withAuthClient(authClient).execute(function(err, result) {
			callback(err, result);
		});
	});
};
...
/**
 * (googleId, function(err, event))
 */
exports.getEvent = function(googleId, callback) {
	loadClientAndAuthClient(null, function(err, client, authClient) {
		if (err) {
			return callback(err);
		}
